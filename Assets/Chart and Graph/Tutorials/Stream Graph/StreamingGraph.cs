...
public class StreamingGraph : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            Graph.DataSource.AddPointToCategory("Player 1", System.DateTime.Now - System.TimeSpan.FromSeconds(x), Mathf.Sin(i) + 10f); // each time we call AddPointToCategory 
            Graph.DataSource.AddPointToCategory("Player 2", System.DateTime.Now  - System.TimeSpan.FromSeconds(x), Mathf.Cos(i)); // each time we call AddPointToCategory 
            x -= Random.value * 3f;
            lastX = x;
        }
    ...
    void Update()
    {
        ...
        if (lastTime + interval < time)
        {
            ...
            Graph.DataSource.AddPointToCategoryRealtime("Player 1", System.DateTime.Now, Mathf.Sin(lastTime) + 10f, 1f); // each time we call AddPointToCategory 
            Graph.DataSource.AddPointToCategoryRealtime("Player 2", System.DateTime.Now, Mathf.Cos(lastTime), 1f); // each time we call AddPointToCategory
        }
